...
function renderWelcomeHtml(name: string | null | undefined): string {
  ...
  return shell(`
    ...
    <p style="margin:0 0 16px 0;">Your email is confirmed and your AlmiKorean account is ready.</p>
...
Your email is confirmed and your AlmiKorean account is ready.
...
function renderSubscriptionHtml(input: {
  ...
  const statusBlock = input.isTrial
    ? `<p style="margin:0 0 16px 0;">Your free trial of ${plan} has started, and you now have full access to every practice feature.</p>
       ${
         trialEndStr
           ? `<p style="margin:0 0 16px 0;">You will not be charged until <strong>${trialEndStr}</strong>. If AlmiKorean isn't right for you, cancel any time before then from your account and you won't pay anything.</p>`
...
function renderSubscriptionText(input: {
  ...
  const g = input.name?.trim()
    ...
    : "Assalam o alaikum,";
  const plan = input.planLabel?.trim() || "AlmiKorean Premium";
  const trialEndStr = formatDateUTC(input.trialEnd);
  const statusBlock = input.isTrial
    ? `Your free trial of ${plan} has started, and you now have full access to every practice feature.\n\n${
        trialEndStr
          ? `You will not be charged until ${trialEndStr}. If AlmiKorean isn't right for you, cancel any time before then from your account and you won't pay anything.`
...
export async function sendSubscriptionConfirmationEmail(input: {
  ...
  const result = await client.emails.send({
    ...
    subject: input.isTrial
      ? "Your AlmiKorean trial has started"
      : "Your AlmiKorean subscription is active",
